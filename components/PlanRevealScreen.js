...
const getPersonalizingCopy = (goal, name) => {
  ...
  const map = {
    ...
    default: {
      ...
      subtitle: "Crafting your custom routine.",
      connecting: "Syncing your profile…",
      calibrating: "Calibrating your plan…",
...
export default function PlanRevealScreen({ onNext }) {
  ...
  // FIX: random dots generated ONCE (no Math.random inside render)
  ...
  // --- Logic Phase 1 ---
  const updateHelperText = (hasConditionOrNone) => {
    // show helper only after they pick something
    setHelperText(hasConditionOrNone ? getHelperCopy(true, goalTitle) : "");
  };
  ...
  // Safer click (prevents crash if onNext missing)
  const handleNext = () => {
  ...
  return (
    <div
      className={`absolute inset-0 w-full h-full flex flex-col transition-colors duration-700 overflow-hidden ${
        phase === "askingHealthInfo" ? "bg-app-background" : "bg-black"
      }`}
    >
      {/* ---------------- PHASE 1: HEALTH INFO ---------------- */}
      {phase === "askingHealthInfo" && (
        <div
          className="flex flex-col h-full w-full animate-in fade-in duration-700 px-5"
          style={{
        ...
        >
          ...
          <div className="mb-2 shrink-0 text-center">
            <h1 className={`text-[26px] font-extrabold text-center ${THEME.textPrimary} mb-1 leading-tight`}>
              {healthCopy.headline}
            </h1>
          ...
          <div className="flex-1 flex flex-col justify-center min-h-0">
                ...
                {CONDITIONS.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-[13px] font-bold text-center leading-tight px-1 transition-colors duration-300 ${isSelected ? THEME.textSelected : THEME.textUnselected}`}>
                        {item.title}
                      </span>
            ...
            {/* Activity */}
            <div className="mt-3">
              <h3 className={`text-[15px] font-bold text-center ${THEME.textPrimary} mb-2`}>
          ...
          <div className="mt-2">
            <button
              ...
              className={`w-full h-14 rounded-full font-bold text-lg text-white transition-all duration-300 active:scale-95 shadow-xl ${
                canContinue
                  ? `bg-gradient-to-b ${THEME.brandGradient} shadow-[#E65473]/30`
          ...
          {showChecklist && (
            <div className="w-full max-w-sm flex flex-col animate-in slide-in-from-bottom-8 duration-700">
              ...
              <div className="mt-6 text-center text-[#E65473] font-medium text-sm animate-pulse">
                {progressPercent === 100
                  ? "Ready!"
                  : "Fine-tuning for: " + (personalizingCopy.checklist[Math.min(3, Math.floor(progressPercent / 25))] || "Results")}
              </div>
            </div>
          )}
      ...
      {phase === "showingTimeline" && (
        <div className="flex flex-col h-full animate-in fade-in duration-1000 bg-black relative">
          {/* FIXED: dots are stable because they come from state, not Math.random in render */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {dots.map((d, i) => (
              <div
                key={i}
                className="absolute bg-white/20 rounded-full w-0.5 h-0.5 animate-ping"
                style={{
                  left: d.left,
                  top: d.top,
