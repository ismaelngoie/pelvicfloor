  ...
  return (
    <div className="w-full h-36 relative my-2">
      <svg className="absolute inset-0 w-full h-full overflow-visible">
        ...
        <g style={{ opacity: show ? 1 : 0, transition: "opacity 1000ms ease 1000ms" }}>
...
export default function PlanRevealScreen({ onNext }) {
  ...
  // ✅ Dark bars only for phases 2 & 3
  const isDarkBars = phase === "personalizing" || phase === "showingTimeline";

  // ✅ Full-bleed into notch/bottom while this screen is mounted
  useSystemBars({
    frameBg: isDarkBars ? "#000000" : "#FAF9FA",
    safeTop: "0px",
    safeBottom: "0px",
    themeColor: isDarkBars ? "#0B0B12" : "#E65473",
    colorScheme: isDarkBars ? "dark" : "light",
  });
  ...
  return (
    <div
      className={`fixed inset-0 md:absolute md:inset-0 w-full h-full flex flex-col transition-colors duration-700 overflow-hidden ${
        phase === "askingHealthInfo" ? "bg-[#FAF9FA]" : "bg-black"
      }`}
    >
      {/* ---------------- PHASE 1: HEALTH INFO ---------------- */}
        ...
        >
          ...
          <div className="flex-1 flex flex-col justify-center min-h-0">
            ...
            <div>
                    ...
                    >
                      ...
                      <span
                        className={`text-[13px] font-bold text-center leading-tight px-1 transition-colors duration-300 ${
                          isSelected ? THEME.textSelected : THEME.textUnselected
                        }`}
                      >
                        {item.title}
                      </span>
              ...
              {/* Helper 1 */}
              <div
                className={`text-center text-xs font-bold ${THEME.helper} transition-opacity duration-300 h-4 mb-2 ${
            ...
            <div className="mt-3">
              ...
              {/* Helper 2 */}
              <div
                className={`text-center text-xs font-bold ${THEME.helper} transition-opacity duration-300 h-4 mt-2 ${
