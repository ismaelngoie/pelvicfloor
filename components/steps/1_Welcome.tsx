...
export default function WelcomeStep() {
  ...
  useEffect(() => {
    const interval = setInterval(
      () => setSocialCount((p) => (p >= 10200 ? 10200 : p + 17)),
      30
    );
  ...
  return (
    // ✅ FIXED overlay bypasses any global layout padding/containers
    <div
      className="
      ...
      "
    >
      {/* ✅ Background fills full screen INCLUDING safe areas */}
      <div
        className="absolute inset-0 -z-10 pointer-events-none"
        style={{
          background:
            "linear-gradient(180deg, #2E1A20 0%, #E6A8B5 45%, #FFFFFF 80%)",
        }}
      />

      {/* ✅ Butterfly layer should also be absolute full-bleed */}
      ...
      <div
        className="
          ...
          w-full max-w-lg mx-auto px-8
          z-10
        "
        // ✅ SAFE AREA + your original pt-24
      ...
      >
        ...
        <div className="w-full space-y-6 animate-slide-up pl-2 shrink-0">
          <BenefitRow
            icon={
              <div className="w-7 h-7 rounded-full bg-[#E65473] flex items-center justify-center">
          ...
          <BenefitRow
            icon={
              <div className="w-7 h-7 rounded-full bg-[#E65473] flex items-center justify-center">
                <MessageCircle className="w-4 h-4 text-white" strokeWidth={3} />
              </div>
            }
      ...
      {/* --- BOTTOM SECTION --- */}
      <div
        className="z-10 w-full max-w-md mx-auto px-8 pt-4"
        // ✅ SAFE AREA + your original pb-12
...
function BenefitRow({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex items-start space-x-4">
      ...
      <p className="text-[#1A1A26] text-[16px] font-semibold leading-snug">{text}</p>
    </div>
  );
}
