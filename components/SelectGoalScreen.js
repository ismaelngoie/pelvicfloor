...
export default function SelectGoalScreen({ onNext }) {
  ...
  return (
    <div className="w-full h-full flex flex-col pt-10 px-6 pb-8 animate-fade-in relative bg-app-background/50">
      ...
      {/* Header - Z-Index 40 (Below selected card, above regular cards) */}
      <div className="z-40 mb-6 shrink-0 relative bg-app-background/80 backdrop-blur-sm py-2">
      ...
      {/* Grid Container */}
      <div className="z-10 flex-1 overflow-y-auto no-scrollbar min-h-0 pt-2 pb-24"> 
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isSelected 
                    ? 'border-app-primary bg-white shadow-2xl shadow-app-primary/20 scale-[1.05] -translate-y-2 z-50' // z-50 forces it on TOP
      ...
      {/* Footer / CTA - Z-Index 60 (Always on top of everything) */}
      <div className="absolute bottom-0 left-0 w-full px-6 pb-8 pt-6 bg-gradient-to-t from-app-background via-app-background/95 to-transparent z-[60]">
        <button 
          onClick={onNext}
          disabled={!selectedId}
