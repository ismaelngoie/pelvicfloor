...
export async function fetchAppTelemetry() {
  ...
  const lifecycle = lifecycleResult.snapshot.docs.map((row) => {
    ...
    return {
      ...
      expirationAt: toDate(value.expirationAt),
      receivedAt: toDate(value.receivedAt),
      appUserId: cleanString(value.appUserId),
      originalAppUserId: cleanString(value.originalAppUserId),
      productId: cleanString(value.productId),
      transactionId: cleanString(value.transactionId),
      originalTransactionId: cleanString(value.originalTransactionId),
      ...
      mediaSource: cleanString(value.mediaSource),
      price: cleanNumber(value.price),
      currency: cleanString(value.currency) || "USD",
      trialConversion: value.trialConversion === true,
      cancelReason: cleanString(value.cancelReason),
      expirationReason: cleanString(value.expirationReason),
    };
  });
