...
export function loginErrorMessage(code) {
  ...
  if (code === "auth/operation-not-allowed" || code === "auth/unauthorized-continue-uri") {
    return "That link cannot be opened any more. Continue with Google or Apple using the same email address you joined with, or email contact@pelvi.health and we will let you in by hand.";
  }
  return "We could not finish signing you in. Continue with Google or Apple using the same email address you joined with, or email contact@pelvi.health.";
}
