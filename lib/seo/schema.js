...
// whether to recommend a $149.99/year health product will not extend the
 ...
 * funnel copy says "500+" because it is ported verbatim from iOS and has to
...
function mobileApplication() {
  ...
  return {
    ...
    // modelled separately below, because putting 149.99 here would tell a
    ...
    //
    // THIS DELIBERATELY DOES NOT MATCH THE PAYWALL, WHICH PRINTS 4.9.
    //
    // The paywall is marketing copy on our own page and the owner sets it. This
    // is different in kind: structured data is a machine readable claim handed
    // to Google to be shown AS VERIFIED FACT, with stars, in results. Google
    // checks it against the source, and the source here is Apple, who publish
    // 4.56 from 41 ratings for app id 6642654729 on a public endpoint.
    //
    // Rounding this up to match the paywall would put a false rating in the one
    // place Google acts on automatically. The penalty is not a lower position on
    // one page, it is a manual action that strips rich results from the whole
    // domain, which would take the eight articles and every schema block down
    // with it. Keeping the true number is what protects the 4.9 on the paywall:
    // a real 4.6 still earns stars in search, and stars are worth more clicks
    // than the extra 0.3 is worth anywhere.
    //
    // If the visible number ever has to match, move the PAYWALL down to the API
    // value. Do not move this one up.
    aggregateRating: {
...
// UN/CEFACT Recommendation 20 unit codes for the billing period, so the Offer a
// crawler and an assistant read carries the same period lib/pricing.js sells.
// "ANN" is a year, "MON" a month. Derived from DEFAULT_PRICE_PERIOD rather than
// typed, because a period that drifted from the paywall here is a false machine
// readable claim in the one place Google acts on automatically.
const PERIOD_UNIT_CODE = { year: "ANN", month: "MON", week: "WEE", day: "DAY" };

function membership() {
  const amount = DEFAULT_PRICE_AMOUNT;
  const periodUnit = PERIOD_UNIT_CODE[DEFAULT_PRICE_PERIOD] || "ANN";
  return {
    "@type": "Product",
    "@id": MEMBERSHIP_ID,
    ...
    offers: {
      ...
      priceSpecification: {
        ...
        price: amount,
        priceCurrency: PRICE_CURRENCY,
        // One billing period at the list price, cancel anytime. The period is
        // whatever lib/pricing.js sells (a year today, "ANN"), which is what the
        // paywall says in words directly above the button.
        billingDuration: 1,
        billingIncrement: 1,
        unitCode: periodUnit,
        referenceQuantity: {
