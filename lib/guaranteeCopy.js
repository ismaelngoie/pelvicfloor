...
import { DEFAULT_PRICE_LABEL, DEFAULT_PRICE_PERIOD } from "./pricing";

export const SESSIONS_PER_WEEK = 5;
export const QUALIFYING_WEEKS_REQUIRED = 11;
export const TOTAL_WEEKS = 13;
export const CLAIM_EMAIL = "contact@pelvi.health";
 ...
 */
export function outcomePhrase(goalId, focusId) {
  switch (goalId) {
    case "intimacy":
      // The web-only "Tighten Vaginal Canal" focus: subjective on purpose, like
      // every other phrase here, but in the word she chose.
      if (focusId === "intimacy.tightening") return "feeling firmer and rejuvenated inside";
...
export function secondSubCTA(day90) {
  const tail = DEFAULT_PRICE_PERIOD === "year" ? " Your whole year back." : "";
  return `And if you're not there by ${day90} after ${SESSIONS_PER_WEEK} sessions a week, we refund the full ${DEFAULT_PRICE_LABEL} you paid.${tail}`;
}
...
export function coverageBody(goalId, focusId) {
  return [
    `• Complete ${SESSIONS_PER_WEEK} sessions a week for at least ${QUALIFYING_WEEKS_REQUIRED} of ${TOTAL_WEEKS} weeks.`,
    `• If you're not ${outcomePhrase(goalId, focusId)} by day 90, email ${CLAIM_EMAIL}.`,
...
export function milestoneCardPromise(goalId, day90, focusId) {
  return `If you're not ${outcomePhrase(goalId, focusId)} by ${day90}, we refund the full ${DEFAULT_PRICE_LABEL} you paid.`;
}
