...
export const FUNNEL_LADDER = {
  ...
  method: "06_method",
  howItHelps: "07_how_it_helps",
  intake: null,
  health: "14_health",
  personalizing: "15_personalizing",
  planReveal: "16_plan_reveal",
  bridge: "17_bridge",
  paywall: "18_paywall",
};

// The six beats inside the Coach Mia intake screen, by sub-step id. Zero
// padded so the "deepest rung" comparison in recordRung stays lexical.
export const INTAKE_LADDER = {
  name: "08_intake_name",
  age: "09_intake_age",
  height: "10_intake_height",
  weight: "11_intake_weight",
  profile: "12_intake_profile",
  story: "13_intake_story",
};

export const CHECKOUT_LADDER = {
  email: "19_checkout_email",
  card: "20_checkout_card",
  paid: "21_paid",
};

// The paid landing pages sit in FRONT of the funnel, one per keyword theme,
// so their rungs are numbered 00 and sort (and depth-compare) below
// "01_landing". A session that entered through the leak page reads
// 00 -> 03 -> 04... in funnel_step, because the goal screen (02) is the
// question her ad click already answered. Filtering sessions by
// funnel_step = 00_lp_leaks against sessions that start at 01_landing is
// exactly the LP-entry versus homepage-entry comparison. Keyed by goal id so
// this table and the `goal` tag join without translation.
export const LANDING_PAGE_LADDER = {
  bladderLeaks: "00_lp_leaks",
};
...
/**
 * A paid theme landing page was viewed. Called once, on mount, from that
 * page's client component; the rung is a different document from the funnel's
 * but the same Clarity session, which is what makes the doorway segmentable.
 */
export function trackLandingPageView(goalId) {
  const rung = LANDING_PAGE_LADDER[goalId];
  if (!rung) return;
  recordRung(rung);
  track(`lp_${goalId}`);
}
