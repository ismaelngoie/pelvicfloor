...
/** The monthly price the app charges. Source: the iOS paywall. */
...
export function computeOverview(members, now = new Date()) {
  ...
  return {
    ...
    monthlyRevenue: billed > 0 ? billed * MONTHLY_PRICE_USD : null,
...
/**
 * Monthly revenue the dashboard can see, at the end of each period.
 *
 * Every member it can see money from who had joined by then, times the monthly
 * price. It is a floor, not a total: web members who paid through Stripe and
 * were never marked by hand are missing from every point on the line. The chart
 * says so out loud.
 */
export function revenueSeries(members, buckets) {
  ...
  return buckets.map((b) => {
    ...
    return count * MONTHLY_PRICE_USD;
  });
}
