 ...
 */
export async function fetchRevenueCatMembers(user, memberIds) {
  ...
  for (let index = 0; index < ids.length || (index === 0 && !ids.length); index += 30) {
    ...
    const response = await fetch("/api/revenuecat-members", {
      ...
      body: JSON.stringify({ customerIds, includeHeadline: index === 0 }),
    });
  ...
  const headline = first.headline || null;
  return {
    source: first.source || "RevenueCat API v2",
    definition: first.definition || "Production App Store subscriptions set to renew",
 ...
 */
export async function fetchRevenueCatOwnerMetrics(user, startDate, endDate, currency = "USD") {
...
function consolidateRevenueCatCustomers(rows) {
  ...
  for (const row of rows) {
    ...
    // Prefer the renewing representation if aliases briefly disagree, then the
    // row carrying the fuller subscription payload. The authoritative headline
    // remains RevenueCat's Subscription Status chart either way.
    const preferred = row.isActivePremium && !current.isActivePremium
      ? row
