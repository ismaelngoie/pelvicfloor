...
export async function fetchRevenueCatMembers(user, memberIds, ownerMetrics = null) {
  ...
  const paidCustomers = activeCustomers.filter((customer) => customer.phase === "paid");
  const canceledPaidCustomers = paidCustomers.filter((customer) => customer.state === "canceled_with_access");
  const paid = ownerMetricValue(ownerMetrics, "activeSubscriptions");
  const headline = Number.isFinite(paid)
    ? {
      paid,
  ...
  return {
    ...
    definition: first.definition || "Production App Store paid subscriptions verified by RevenueCat",
    fetchedAt: Math.max(...batches.map((batch) => Number(batch.fetchedAt) || 0), Date.now()),
    projectId: first.projectId || "",
    headline,
    customers,
    totals: {
      paid: Number.isFinite(headline?.paid) ? headline.paid : paidCustomers.length,
      syncedPaid: paidCustomers.length,
      activeAccess: activeCustomers.length,
      legacyAccess: activeCustomers.length - paidCustomers.length,
      canceledPaidWithAccess: canceledPaidCustomers.length,
 ...
 * per install and cost per first payment mathematically comparable.
 */
