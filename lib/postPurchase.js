...
//
// WHAT IS STORED, AND WHAT IS NOT. Her goal id, her focus id and her first
// name. Nothing else. No email, no age, no health answers, no conditions: none
// of it is needed to render the 90-day screen, and a health condition sitting
// in localStorage for months is a liability with no upside. The focus is there
// for one reason: a member who tapped "Tighten Vaginal Canal" must read that
// word after paying too, and the member record is written to from /app.
//
...
import { DEFAULT_PRICE_PERIOD } from "./pricing";
 ...
 *
 * @param {{goalId?: string|null, focusId?: string|null, name?: string}} profile
 */
export function rememberPlan(profile) {
  if (typeof window === "undefined") return;
  const goalId = typeof profile?.goalId === "string" ? profile.goalId : null;
  const focusId = typeof profile?.focusId === "string" ? profile.focusId : null;
  ...
  try {
    window.localStorage.setItem(
      PLAN_KEY,
      JSON.stringify({ v: 1, goalId, focusId, name, savedAt: Date.now() })
    );
  } catch {
...
export function rememberPrice({ amount, currency, interval } = {}) {
  ...
  try {
    ...
    const base = saved && saved.v === 1 ? saved : { v: 1, goalId: null, focusId: null, name: "" };
    window.localStorage.setItem(
      PLAN_KEY,
      JSON.stringify({
        ...base,
        price: { amount, currency: currency || "usd", interval: interval || DEFAULT_PRICE_PERIOD },
        savedAt: Date.now(),
      })
    );
  } catch {
...
export function readPlan() {
  ...
  try {
    ...
    const focusId = typeof saved.focusId === "string" ? saved.focusId : null;
    ...
    const price =
      saved.price && typeof saved.price.amount === "number" && Number.isFinite(saved.price.amount)
        ? {
            ...
            interval:
              typeof saved.price.interval === "string"
                ? saved.price.interval
                : DEFAULT_PRICE_PERIOD,
          }
        : null;
    if (!goalId && !name && !price) return null;
    return { goalId, focusId, name, price };
  } catch {
    return null;
  }
}
