...
//
// The web-only "Tighten Vaginal Canal" focus (components/funnel/appCopy.js
// TIGHTENING_FOCUS) rewrites the intimacy line wherever a member who chose it
// would otherwise read "intimacy": her focus is the word she came for.

const TIGHTENING_FOCUS = "intimacy.tightening";
const tightening = (goalId, focusId) => goalId === "intimacy" && focusId === TIGHTENING_FOCUS;
...
export function libraryGoalPhrase(goalId, focusId) {
  if (tightening(goalId, focusId)) return "vaginal rejuvenation";
  switch (goalId) {
...
export function pathwayTitle(goalId, focusId) {
  if (tightening(goalId, focusId)) return "The 90-Day Vaginal Tightening Journey";
  switch (goalId) {
...
export function pathwaySubtitle(goalId, focusId) {
  if (tightening(goalId, focusId)) return "Rejuvenated, firmer, more feeling. Guaranteed.";
  switch (goalId) {
...
export function goalFeelingQuestion(goalId, focusId) {
  if (tightening(goalId, focusId)) return "How firm and toned do you feel inside today?";
  switch (goalId) {
...
/**
 * The title over the goal-feeling trend on the "How You're Feeling" screen,
 * phrased for her goal. These are the strings ProgressDashboardViewModel picks
 * in DailySymptomView.swift, word for word, so the chart is called the same
 * thing on both devices.
 */
export function goalFeelingTitle(goalId, focusId) {
  if (tightening(goalId, focusId)) return "Tone and Feel";
  switch (goalId) {
    case "intimacy": return "Body Connection";
    case "bladderLeaks": return "Control Confidence";
    case "postpartum": return "Recovery Strength";
    case "pregnancyPrep": return "Body Readiness";
    case "pelvicPain": return "Ease and Comfort";
    default: return "Core Strength Feel";
  }
}
...
export function goalPrompt(goalId, focusId) {
  if (tightening(goalId, focusId)) return "How does this rejuvenate my vagina?";
  switch (goalId) {
