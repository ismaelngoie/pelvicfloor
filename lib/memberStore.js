...
export async function findMemberByEmail(email) {
  ...
  const snap = await getDocs(
    query(collection(db(), "users"), where("email", "in", spellings), limit(10))
  );
  ...
  return rows.find((row) => row.platform === "ios") || rows[0];
}
...
export async function resolveMember(user) {
  ...
  const existing = email ? await findMemberByEmail(email) : null;
  if (existing) {
